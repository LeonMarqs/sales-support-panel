...
public class CampaignResolver {
	...
	public List<GetCampaignsUseCase.Output> getCampaigns(@Argument Optional<GetCampaignsFilterDTO> filter) {
		return getCampaignsUseCase.execute(new GetCampaignsUseCase.Input(filter.map(GetCampaignsFilterDTO::name).orElse(null),
				filter.map(GetCampaignsFilterDTO::startDate).orElse(null),
				filter.map(GetCampaignsFilterDTO::endDate).orElse(null), filter.map(GetCampaignsFilterDTO::budget).orElse(null)));
	}

	@MutationMapping
	public CreateCampaignUseCase.Output createCampaign(@Argument String name, @Argument String startDate,
			@Argument String budget) {
		return createCampaignUseCase.execute(
				new CreateCampaignUseCase.Input(name, startDate != null ? LocalDate.parse(startDate) : null,
						budget != null ? new BigDecimal(budget) : null));
	}
